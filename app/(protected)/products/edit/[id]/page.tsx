...
export default function ProductsPage() {
  ...
  const {data : product, isLoading, isSuccess, isError} = useQuery<Product>({
    queryKey : ['product'], 
    queryFn : async () => {
  ...
  useEffect(() =>{
    if(isSuccess && product){
      setNewProduct(prev => ({
        ...
        sizes : product.sizes.map(product => ({ sizeId: product.sizeId,
      ...
      <h1 className="text-3xl font-bold mb-6">Add New Product</h1>
          ...
          <div className='flex justify-end mt-10'>
            <Button onClick={saveProduct} className="" disabled={saveProductMutation.isLoading}>
              {saveProductMutation.isLoading ? "Updating..." : "Update product"}
            </Button>
          </div>
    </div>
  )
